...
const TransactionRow = ({ transaction, balanceAfter, onEdit, onDelete }) => {
    ...
    return (
        <div className="bg-gray-800 lg:bg-white border-b border-gray-700 lg:border-gray-200 hover:bg-gray-700 lg:hover:bg-gray-50">
                ...
                <div className="flex justify-between items-center">
                    ...
                    <div className="text-right flex-shrink-0">
                        ...
                        <p className="text-sm text-gray-400">
                            Balance: <span className="font-medium text-gray-200">${balanceAfter.toFixed(2)}</span>
                        </p>
                    </div>
                </div>
            </div>

            {/* --- Desktop Table Row Layout (Unchanged) --- */}
            <div className="hidden lg:grid grid-cols-12 gap-4 items-center p-3">
                ...
                <div className="col-span-3 text-sm font-medium text-gray-800">{transaction.details}</div>
                ...
                <div className="col-span-1 text-sm font-semibold text-gray-800 text-right">{balanceAfter.toFixed(2)}</div>
...
const TransactionModal = ({ show, onSave, onClose, type, initialData }) => {
    // ... (This component is unchanged as it works well on all screen sizes)
...
export default function Wallet({ book, onBack }) {
    ...
    const { totalCashIn, totalCashOut, netBalance, transactionsWithBalance } = useMemo(() => {
        let cashIn = 0, cashOut = 0;
        // The original logic correctly calculates running balances for a chronological view
        const reversedTx = [...transactions].reverse();
        let runningBalance = 0;
        
        const txWithBalance = reversedTx.map(tx => {
            if (tx.type === 'cash-in') {
        ...
        return {
            ...
            transactionsWithBalance: txWithBalance.reverse()
        };
    }, [transactions]);

    const groupedTransactions = useMemo(() => {
      return transactionsWithBalance.reduce((acc, tx) => {
            ...
            <div className="bg-gray-900 lg:bg-gray-100 min-h-screen">
                ...
                <div className="pb-24 lg:pb-0"> {/* Padding bottom for mobile to avoid overlap with FAB */}
                        ...
                        {Object.keys(groupedTransactions).length > 0 ? (
                            Object.entries(groupedTransactions).map(([date, txs]) => (
                                <div key={date}>
                                    ...
                                    {txs.map((tx) => (
                                        <TransactionRow key={tx.id} transaction={tx} balanceAfter={tx.balanceAfter} onEdit={handleOpenModalForEdit} onDelete={handleDeleteTransaction} />
                                    ))}
                                </div>
                            ))
                        ) : (
                    ...
                    <div className="hidden lg:block bg-white rounded-t-lg lg:rounded-lg border-t lg:border lg:shadow-sm lg:mx-6">
                         ...
                         <div className="hidden lg:grid grid-cols-12 gap-4 bg-gray-50 p-3 font-semibold text-xs text-gray-500 uppercase border-b">
